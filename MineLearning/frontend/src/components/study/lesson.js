...
class Lesson_Component extends Component {
    constructor(props) {
        super(props);
        this.state = {
            lesson: null,
        ...
        GetLessons().then(response => {
            this.setState({
                lesson: response.data.map((post, i) => (
                    ...
                    {post.id == id+1 &&
                        <tr>
                        <td><img src={post.poster} className="lessimg"/>
                        </td>
                        <td><div className="lesson_name">{post.title}</div>
                        <div className="lesson_preview" dangerouslySetInnerHTML={createMarkup(post.description)}></div>
                        </td>
                        </tr>
                        }
...
export default Lesson_Component;